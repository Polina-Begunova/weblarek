...
export class ContactsForm extends Form<IBuyer> {
    ...
    constructor(container: HTMLElement, actions?: IContactsActions) {
        ...
        // Сохраняем данные при их изменении через onInputChange
        if (actions?.onInputChange) {
            ...
            actions.onInputChange = (field: keyof IBuyer, value: string) => {
                // Сохраняем данные в локальном состоянии с правильной типизацией
                if (field === 'payment') {
                    // Проверяем, что значение payment соответствует типу
                    if (value === 'card' || value === 'cash') {
                        this._currentData[field] = value;
                    }
                } else {
                    // Для остальных полей просто присваиваем string
                    this._currentData[field] = value;
                }
        ...
        // Обработчики изменений для email и телефона
    ...
    private getFormData(): IBuyer {
        // Используем сохраненные данные из _currentData и текущие значения полей
        return {
    ...
    render(data?: Partial<IBuyer>): HTMLElement {
        ...
        if (data) {
            // Сохраняем все переданные данные
